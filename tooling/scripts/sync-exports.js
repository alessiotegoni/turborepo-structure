...
if (
  ...
  !fs.statSync(absoluteTargetDir).isDirectory()
) {
...
const relativeToPackage = path
  .relative(packageDir, absoluteTargetDir)
  .replace(/\\/g, "/");
...
if (JSON.stringify(pkg.exports[subpath]) !== JSON.stringify(exportEntry)) {
  ...
  const orderedExports = Object.keys(pkg.exports)
    .sort()
    .reduce((obj, key) => {
  ...
  console.log(
    `✅ Aggiornato package.json in ${path.relative(process.cwd(), packageJsonPath)} con export "${subpath}"`,
  );
} else {
